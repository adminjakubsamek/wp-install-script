...
- `$UserDesktopShortcuts` — smazatelné kopie zástupců na plochu: Chrome, Firefox, Outlook, Word, Excel, TeamViewer, **Adobe Acrobat Reader** a **PDFsam Basic**. Alternativní názvy se oddělují `|` (použije se první nalezený) — Adobe pojmenovává zástupce podle verze různě.
...
- **Čekající restart se rozlišuje na „hard" a „soft"** — `RebootPending` (CBS), `RebootRequired` (Windows Update) a čekající přejmenování počítače jsou tvrdé signály, u kterých se Office rovnou odloží za restart. Samotné `PendingFileRenameOperations` po sobě nechá skoro každý instalátor, takže se Office přesto zkusí nainstalovat a odloží se teprve při selhání.
...
- **Rozsypané znaky ve výpisu wingetu** — winget píše UTF-8, ale konzole má národní znakovou stránku (CP852/CP850), takže se z bloků průběhu `█▒░` stanou patvary `ÔûêÔûê`. Projeví se to jen tehdy, když se výstup **přesměrovává** (uvnitř funkce); psaný přímo na konzoli se zobrazí správně. Skript proto na dobu volání přepne `[Console]::OutputEncoding` na UTF-8 a po doběhnutí ji vrátí zpět.
- **Průběh stahování se do logu nezapisuje** — řádky s pruhem průběhu, procenty a otáčejícím se ukazatelem se filtrují. Bez toho měl log přes 1900 řádků místo zhruba 300 a chyby se v něm ztrácely.
